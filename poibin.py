...
Modified on Sat Dec 22, 2017 by Nate Pope -- added np.real(...) on line 214
 to control numerical error in FFT (else it may seem xi has non-zero imaginary part
 when in reality the imaginary part is on the order of 1e-15 or so)
...
# ------------------------------------------------------------------------------
    ...
    def get_pmf_xi(self):
        ...
        xi = np.fft.fft(chi)
        xi = np.real(xi) # NATE: added, b/c of floating point error in imaginary part
