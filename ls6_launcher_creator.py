...
$LAUNCHER_DIR/paramrun $EXECUTABLE $CONTROL_FILE

'''

parametric_job_submission_ls6='''
...
def generate_lonestar_launcher(results, launcher_file):
    ...
    # Must account (on Lonestar6) for normal having 128 threads per node;
...
def generate_ls6_launcher(results, launcher_file):
    """
    Write a Lonestar 6 launcher SLURM script.
    """

    # Must account (on Lonestar 6) for normal having 128 threads per node
#    if results.queue == 'largemem':
#        threads_per_node = 32
...
#    else:
    ...
    if results.job:
        launcher_file.write(parametric_job_submission_ls6.format(jobfile=results.job))


def check_time_and_queue(results):
    host = subprocess.check_output(["hostname", "-f"]).split(b'.')[1]
...
Report problems to rt-other@ccbb.utexas.edu''')
    ...
    generate_ls6_launcher(results, launcher_file)
    launcher_file.close()
