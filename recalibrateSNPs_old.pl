...
non-parametric SNP quality recalibration based on what is observed in a 
set of SNPs that give reproducible genotypes in replicates 
...
for DP, both very low and very high values are considered bad, the best values
...
Both input files must contain DP, SB and AB INFO fields.
...
if ("@ARGV"=~/-nosb/) { $nosb=1;}
if ("@ARGV"=~/-nodp/) { $nodp=1;}
if($nodp+$nosb+$noab==3) { die "no fields left to filter!\n";}
...
#			if ($dpQ>50) { $dpQ=100-$dpQ;}						
	...
	push @joint, sprintf("%.5f",($dpQ/100)*($abQ/100)*($sbQ/100));
}
close TR;
...
#			if ($dpQ>50) { $dpQ=100-$dpQ;}						
	...
	my $jt=sprintf("%.5f",($dpQ/100)*($abQ/100)*($sbQ/100));
#warn "\t$jt\n";
