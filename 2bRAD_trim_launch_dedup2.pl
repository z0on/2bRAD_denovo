...
site=[pattern] perl-style pattern of the restriction site to recognise, in single quotes.
		default is BcgI: \'.{12}CG.{8}GC.{12}|.{12}GC.{8}CG.{12}\'
...
my $site=".{12}CG.{8}GC.{12}|.{12}GC.{8}CG.{12}";
if("@ARGV"=~/site=(\S+)/){ $site=$1;}
my $sampleid=100;
if("@ARGV"=~/sampleID=(\d)/){ $sampleid=$1;}
my $adaptor="[ATGCN][ATGCN][ATGCN][ATGCN][ATGCN]";
...
foreach $fqf (@fqs) {
	...
	print "trim2bRAD_2barcodes_dedup2.pl input=$fqf site=\"$site\" adaptor=\"$adaptor\" sampleID=$sampleid\n";
}
