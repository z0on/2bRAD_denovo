...
def generate_stampede_launcher(results, launcher_file):
    ...
    # Must account (on Stampede2) for normal having 64 or 48 (skx) threads per node
    if results.queue in ['skx-normal','skx-dev','skx-large'] :
        threads_per_node = 48
    else:
        threads_per_node = 64
    ...
    if not results.num_nodes:
        ...
        if results.wayness:
            ...
            num_nodes, remainder = divmod(num_jobs, threads_per_node)
            if remainder != 0:
                num_nodes += 1                
    else:
        # A number of nodes has been specified. Default wayness is 48 (fast skx nodes) or 64 (slow knl nodes) on Stampede2. Use specified wayness to calculate total tasks.
        num_nodes = results.num_nodes
        num_nodes_line = '#SBATCH -N {}\n'.format(num_nodes)
...
def generate_ls5_launcher(results, launcher_file):
    ...
    if not results.num_nodes:
        ...
        # A number of nodes has been specified. Default wayness is 64 (knl) or 48 (skx) on Stampede2. Use specified wayness to calculate total tasks.
        num_nodes = results.num_nodes
        num_nodes_line = '#SBATCH -N {}\n'.format(num_nodes)
...
def check_time_and_queue(results):
    ...
    queues = {'ls5':['normal','development','largemem','gpu','vis','serial'],
              'stampede2':['normal','development','largemem','skx-normal','skx-large','skx-dev','normal-mic','gpu','gpudev','vis']}
    ...
    elif host == "stampede2" and results.queue in ['development','skx-dev']: time_limit = 2
    elif host == "stampede2" and \
        results.queue in ['gpudev','visdev']:                   time_limit = 4
    elif host == "stampede2" and results.queue == 'vis':         time_limit = 8
    elif host == "stampede2" and results.queue == 'serial':      time_limit = 12
    elif host == "stampede2" and \
        results.queue in ['normal','largemem','normal-mic','skx-normal','skx-large']:    time_limit = 48
