...
version of trimmer/deduplicator launcher script allowing for Ns instead of 'CC' in the upstream tag
...
foreach $fqf (@fqs) {
	...
	print "trim2bRAD_2barcodes_dedup_N2.pl input=$fqf site=\"$site\" adaptor=\"$adaptor\" sampleID=$sampleid deduplicate=1 bc=$bcod\n";
}
