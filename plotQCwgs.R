...
c5=1-cumul[2,]
names(c5)=bams
print("proportion of sites with at least 2x coverage for each sample:")
logq=log(c5+1e-5)
zq=(logq-mean(logq))/sd(logq)
goods=data.frame(cbind(names(zq[zq>(-3)])))
print(data.frame(cbind(sort(c5))))
write.table(data.frame(cbind(sort(c5))),file=paste(fin,".quality.txt",sep=""),quote=F,col.names=F)
